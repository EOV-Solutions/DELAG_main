#!/usr/bin/env python3
"""
MODIS Reconstruction Analysis Script

This script performs comprehensive analysis of MODIS ground truth images vs reconstructed images:
...
class MODISReconstructionAnalyzer:
    """Comprehensive analyzer for MODIS ground truth vs reconstructed images with time-averaged analysis."""
    ...
    def __init__(self, roi: str, gt_folder: str, recon_folder: str, output_dir: str):
        ...
        Args:
            ...
            gt_folder: Path to ground truth MODIS images
        ...
        self.gt_pattern = r".*_(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})\.tif"
    ...
    def _create_batch_comparison_plot(self, date_batch: List[date], batch_idx: int):
        ...
        # Add colorbar
        # fig.colorbar(im2, ax=axes.ravel().tolist(), orientation='vertical', 
        #             fraction=0.046, pad=0.04, label='LST (°K)')
        
        plt.suptitle(f'LST Comparison - Batch {batch_idx + 1} ({self.roi})', fontsize=14)
        plt.tight_layout()
    ...
    def create_time_series_plot(self, num_pixels: int = 5):
        ...
        plt.suptitle(f'LST Time Series - Random Pixels ({self.roi})', fontsize=14)
        plt.tight_layout()
    ...
    def create_error_distribution_plots(self, gt_data: np.ndarray, recon_data: np.ndarray, date_str: str):
        ...
        fig.suptitle(f'Error Distribution Analysis - {date_str} ({self.roi})', fontsize=16)
        ...
        try:
            r2_val = r2_score(gt_valid, recon_valid)
            if np.isnan(r2_val):
                r2_val = 0.0
        except:
            r2_val = 0.0
        axes[1, 0].set_title(f'GT vs Reconstructed (R² = {r2_val:.3f})')
        ...
        plt.tight_layout()
        plt.savefig(self.advanced_viz_dir / f'error_distribution_{date_str}.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_error_maps(self, gt_data: np.ndarray, recon_data: np.ndarray, date_str: str):
        """Create spatial error maps."""
        errors = gt_data - recon_data
        valid_mask = ~(np.isnan(gt_data) | np.isnan(recon_data))
        
        fig, axes = plt.subplots(1, 3, figsize=(18, 6))
        fig.suptitle(f'Spatial Error Analysis - {date_str} ({self.roi})', fontsize=16)
        
        # 1. Error map
        # Calculate symmetric limits for centered colormap
        error_max = np.nanmax(np.abs(errors))
        im1 = axes[0].imshow(errors, cmap='RdBu_r', vmin=-error_max, vmax=error_max)
        axes[0].set_title('Error Map (GT - Reconstructed)')
        axes[0].set_xticks([])
        axes[0].set_yticks([])
        plt.colorbar(im1, ax=axes[0], label='Error (°K)')
        ...
        axes[1].set_xticks([])
        axes[1].set_yticks([])
        plt.colorbar(im2, ax=axes[1], label='|Error| (°K)')
        ...
        plt.tight_layout()
        plt.savefig(self.advanced_viz_dir / f'error_maps_{date_str}.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_correlation_analysis(self, gt_data: np.ndarray, recon_data: np.ndarray, date_str: str):
        ...
        fig.suptitle(f'Correlation Analysis - {date_str} ({self.roi})', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig(self.advanced_viz_dir / f'correlation_analysis_{date_str}.png', 
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_analysis(self, min_percentage: float = 25.0, images_per_plot: int = 10, 
        ...
        print(f"=== MODIS Reconstruction Analysis for {self.roi} ===")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive MODIS reconstruction analysis",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python modis_reconstruction_analysis.py --roi vietnam --gt-folder data/retrieved_data/vietnam/movis --recon-folder data/output/vietnam/reconstructed_lst_train --output-dir results/vietnam
  
  python modis_reconstruction_analysis.py --roi thailand --min-percentage 30 --images-per-plot 5 --num-pixels 3
  
  python modis_reconstruction_analysis.py --roi cambodia --no-advanced-viz  # Disable advanced visualizations
        """
    )
    
    parser.add_argument("--roi", required=True, help="Region of Interest identifier")
    parser.add_argument("--gt-folder", required=True, 
                       help="Path to ground truth MODIS images folder")
    ...
    analyzer = MODISReconstructionAnalyzer(
        roi=args.roi,
