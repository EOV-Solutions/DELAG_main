# SMW (Statistical Mono-Window) algorithm coefficients for different Landsat satellites
# These coefficients are used to compute Land Surface Temperature (LST) from thermal bands
...
def get_coefficients(landsat: str) -> list:
    """
    Get SMW coefficients for a specific Landsat satellite.
    
    Args:
      landsat (str): Landsat satellite id ('L4', 'L5', 'L7', 'L8', or 'L9')
    
    Returns:
      list: List of coefficient dictionaries
    """
    if landsat == 'L4':
        return COEFF_SMW_L4
    elif landsat == 'L5':
        ...
        raise ValueError(f"Unsupported Landsat satellite: {landsat}")

def get_coefficient_by_tpwpos(landsat: str, tpwpos: int) -> dict:
    """
    Get SMW coefficients for a specific Landsat satellite and TPW position.
    ...
    Returns:
      dict: Coefficient dictionary with A, B, C values
    """
    coeffs = get_coefficients(landsat)
    for coeff in coeffs:
        if coeff['TPWpos'] == tpwpos:
            return coeff
    raise ValueError(f"TPW position {tpwpos} not found for Landsat {landsat}")
